...
CONTROL_DIR="$TEMP_ROOT/control"; CHECKOUT="$TEMP_ROOT/twitterauth app's [gate] \"quoted\" \`touch TWITTERAUTH_ROOT_MARKER\`"; ATTACKER_ROOT="$TEMP_ROOT/attacker"; LOG="$TEMP_ROOT/commands.log"; SHELL_LOG="$TEMP_ROOT/shell.log"
...
rm -f "$LOG"; (cd "$CONTROL_DIR"&&TWITTERAUTH_COMMAND_LOG="$LOG" /usr/bin/make --no-print-directory -f "$MAKEFILE" "PYTHON=$FAKE_PYTHON" check) >/dev/null 2>&1; grep -Fq "$FAKE_PYTHON" "$LOG"; [ ! -e "$CONTROL_DIR/TWITTERAUTH_ROOT_MARKER" ]; [ ! -e "$CONTROL_DIR/TWITTERAUTH_PYTHON_MARKER" ]
...
printf '%s\n' 'Make authority tests passed: 30 target/authority cases, literal hostile Python path, 2 raw Make-syntax rejections, 2 MAKEFILE_LIST rejections, 2 startup-boundary cases, caller MAKEFLAGS rejection, and 10 mode rejections'
